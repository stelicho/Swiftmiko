...
extension NIOSSHSignature {
    ...
    internal enum BackingSignature: Sendable {
        ...
        // Fork addition: ssh-rsa signature verification (RFC 4253 §6.6).
        // There is no structured signature type for RSA here either — the
        // wire format is just the raw PKCS#1-v1.5 signature bytes.
        case rsa(RawBytes)
    ...
    /// The prefix of an RSA signature.
...
extension ByteBuffer {
    ...
    mutating func writeSSHSignature(_ sig: NIOSSHSignature) -> Int {
        ...
        case .rsa(let sig):
            return self.writeRSASignature(signatureBytes: sig)
        }
    }

    private mutating func writeRSASignature(signatureBytes: NIOSSHSignature.BackingSignature.RawBytes) -> Int {
        // The ssh-rsa signature format is the algorithm prefix, followed by the
        // raw PKCS#1-v1.5 signature bytes as a single string.
    ...
    mutating func readSSHSignature() throws -> NIOSSHSignature? {
        try self.rewindOnNilOrError { buffer in
            ...
            } else if bytesView.elementsEqual(NIOSSHSignature.rsaSignaturePrefix) {
                return try buffer.readRSASignature()
            } else {
    ...
    /// Not safe to call from arbitrary code as this does not return the reader index on failure: it relies on the caller performing
    /// the rewind.
    private mutating func readRSASignature() throws -> NIOSSHSignature? {
        // For ssh-rsa the signature is just the raw PKCS#1-v1.5 signature bytes, encoded as a String.
        guard let sigBytes = self.readSSHString() else {
            return nil
        }

        return NIOSSHSignature(backingSignature: .rsa(.byteBuffer(sigBytes)))
    }
