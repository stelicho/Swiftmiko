...
// Fork addition: RSA host key verification (see the .rsa case below).
...
extension NIOSSHPublicKey {
    ...
    internal func isValidSignature<DigestBytes: Digest>(_ signature: NIOSSHSignature, for digest: DigestBytes) -> Bool {
        ...
        case (.rsa(let key), .rsa(let sig)):
            // ssh-rsa (RFC 4253 §6.6) signatures are "RSASSA-PKCS1-v1_5 using
            // the SHA-1 hash algorithm" — that phrase names SHA-1 as the hash
            // the PKCS#1 v1.5 *signing primitive itself* applies to its input
            // message, not a description of what `digest` already contains.
            // `digest` here is H, the KEX exchange hash (which, depending on
            // the negotiated KEX method, may not even be a SHA-1 value) —
            // it's the *message* being signed, and per the PKCS#1 v1.5
            // scheme that message still gets hashed with SHA-1 before
            // padding/encryption. swift-crypto's digest-based
            // isValidSignature(_:for:padding:) treats its argument as an
            // already-final digest with no further hashing, so we have to
            // do that SHA-1 pass ourselves here. Skipping it (passing H
            // straight through) verifies against the wrong value — this
            // was confirmed empirically against a real Cisco IOS ssh-rsa
            // signature: it signs SHA-1(H), not H.
            var rehash = Insecure.SHA1()
            digest.withUnsafeBytes { rehash.update(bufferPointer: $0) }
            let rehashedDigest = rehash.finalize()
            switch sig {
            case .byteBuffer(let buf):
                return key.isValidSignature(
                    _RSA.Signing.RSASignature(rawRepresentation: buf.readableBytesView),
                    for: rehashedDigest,
                    padding: .insecurePKCS1v1_5
                )
            case .data(let d):
                return key.isValidSignature(
                    _RSA.Signing.RSASignature(rawRepresentation: d),
                    for: rehashedDigest,
                    padding: .insecurePKCS1v1_5
                )
            }
    ...
    internal func isValidSignature(_ signature: NIOSSHSignature, for bytes: ByteBuffer) -> Bool {
        ...
        case (.ed25519, _),
            ...
            (.ecdsaP521, _),
            (.rsa, _):
            // Not exercised by this fork: RSA host keys are only ever
            // verified via the digest-based overload above (KEX exchange
            // hash verification).
            return false
        }
    }
...
extension NIOSSHPublicKey {
    ...
    internal enum BackingKey {
        ...
        // Fork addition: verify-only RSA host key support (RFC 4253 §6.6),
        // for servers too old to offer ed25519/ECDSA host keys.
...
/// Encodes a PKCS#1 `RSAPublicKey` DER structure (`SEQUENCE { modulus INTEGER, publicExponent INTEGER }`)
/// from raw unsigned big-endian magnitude bytes.
///
/// This exists so `_RSA.Signing.PublicKey`'s DER-based initializer (which accepts key sizes down to
/// 1024 bits via `unsafeDERRepresentation`) can be used instead of the raw `init(n:e:)` initializer,
/// which refuses anything below 2048 bits — exactly the key sizes many old SSH servers still use.
private func pkcs1RSAPublicKeyDER(modulus: [UInt8], exponent: [UInt8]) -> [UInt8] {
    ...
    func derInteger(_ magnitude: [UInt8]) -> [UInt8] {
        var value = magnitude.isEmpty ? [0] : magnitude
        if value[0] & 0x80 != 0 {
            value.insert(0, at: 0)
        }
...
extension ByteBuffer {
    ...
    private mutating func writeRSAPublicKey(baseKey: _RSA.Signing.PublicKey) -> Int {
        // For ssh-rsa (RFC 4253 §6.6), the key format is `mpint e` followed by `mpint n`.
        // getKeyPrimitives() only fails on internal extraction errors, which shouldn't
        // happen for a key we (or the peer) already successfully constructed.
    ...
    private mutating func readRSAPublicKey() throws -> NIOSSHPublicKey? {
        ...
        func stripLeadingZeroBytes(_ view: ByteBufferView) -> [UInt8] {
            var bytes = Array(view)
        ...
        let e = stripLeadingZeroBytes(eBytes.readableBytesView)
        let n = stripLeadingZeroBytes(nBytes.readableBytesView)

        // _RSA.Signing.PublicKey(n:e:) refuses anything below 2048 bits, which
        // would reject exactly the old, small host keys this fork exists to
        // support. Route through a hand-built PKCS#1 DER blob instead: the
        // "unsafe" DER initializer accepts key sizes down to 1024 bits.
        let der = pkcs1RSAPublicKeyDER(modulus: n, exponent: e)
